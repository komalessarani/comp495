...
def image_downloader(img_url: str):
    ...
    """
    download_location = "./data/faceScrubImg/male"
    image_name = getImgName(img_url)
    if not (path.exists(download_location + '/'+image_name)):
        try:
...
run_downloader(num_process, m_urls)


end = time.time()
print('Time taken to download {}'.format(len(m_urls)))
print(end - start)
